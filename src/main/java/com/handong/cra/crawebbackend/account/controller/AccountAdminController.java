...
public class AccountAdminController {
    ...
    public ResponseEntity<Void> addUserAuthById(@RequestParam final Long userId, @RequestParam final Integer authOption) {
        accountService.addUserAuthById(userId, UserRoleEnum.values()[authOption]);
        return ResponseEntity.ok().build();
    }

    @DeleteMapping("/users/auth") // 유저 권한 삭제
    public ResponseEntity<Void> removeUserAuthById(@RequestParam final Long userId, @RequestParam final Integer authOption) {
